...
| R2 | 单体热点：验证套件 16,010 行 / `screen_mirror.py` 8,046 行 / `protocol.py` 的 `Handler` 类 ~846 行 / `setting.html` 2,908 行 | 可维护性、可审计性 | 高 | 高 | **部分已修**（2026-09-25：其中"**门控散落在 if/elif**"这一支已收进声明式路由表，见下面 R2「已修」一节 + 套件 Part 51；剩下的行数热点仍在） |
...
**后续（2026-09-25，Part 51）**：这一族的**可审计性**后来单独收了一次 —— 门从"每条分支自己判"变成 `Handler.POST_ROUTES` 表里的一个名字，于是"所有写端点至少受管理门"这件事由**构造**成立、并被一条按表循环的用例枚举校验，而不是靠人记得每一条。见下面 R2「已修」一节。
...
### 已修（2026-09-25，第 3 条验收）：门控成为数据 —— `Handler.POST_ROUTES`

`POST()` 那 ~150 行 if/elif 换成了**一张表 + 一个分发器**：

- `POST_ROUTES` = 16 行 `(字段, 门, 方法名)`，**tuple 而不是 dict，因为顺序即语义**（一次请求带两个 action 字段时，排在前面那条赢，后面那条根本不跑 —— 与旧链的"第一个匹配的 elif 赢"逐行等价，这次改造**没有改任何一条的判定**）。
- `FILE_ROUTES` 管上传（按字段名，因为投文件、投字幕、装插件 POST 到同一个路径），未登记的文件字段吃 `FILE_GATE_DEFAULT`。
- 三条门名 `GATE_MANAGEMENT` / `GATE_TOKEN` / `GATE_CODE`，各自的 403 文案收在 `GATE_REFUSALS`：**一条路由不能自己写拒绝文案，也不能不写**。检查是累积且有序的，所以局域网无令牌的请求听到的仍是管理那句，与旧文案逐字一致。
- `_MANAGEMENT_PARAMS` / `_CODE_EXECUTION_PARAMS` 改为**从表派生**，不再手抄。（顺带记一条：旧的字面名单只有 12 条，`cast-uri`、`set-subtitle-show`、`clear-play-history`、`clear-log` 不在其中，因为它们各自在分支里就地调 `_management_allowed()`；派生名单把它们算进来了 —— 判定不变，但别再拿这份名单反推"旧版谁走了顶部那道批量检查"。）
- 未知门名 **fail closed**：表里打错一个门，那个端点谁都进不来，而不是谁都进得来。

**这一趟顺手抓到的真 bug（不是重构的副产品，是原来的行为）**：旧 `POST()` 走完所有 `elif` 后返回
`{'code': 0, 'message': 'success'}`。于是**字段名拼错**、或者**页面加了一个后端从没听过的按钮**，
用户读到的是"已保存"而什么都没发生 —— 正是 §4.2 / §4.13 那一族"沉默的谎"。现在回 `code: 1`、
把不认识的字段**念回给调用方**，并 `logger.error` 一条。

**套件 Part 51（24 条）**，它测的不是"某条端点有没有门"，而是"**门这条规则还活在结构里吗**"：

- 表的良构：字段不重复、是 tuple、每一行的门名与方法名都存在、两份派生名单恰好等于按门筛出来的结果。
- **AST 扫描（范围限定在 `class Handler` 内）**：`POST` 恰好一个、不直接叫任何门（`_same_site` 除外，它就是那条链的第一问）、必须读 `POST_ROUTES` / `FILE_ROUTES` / `_gate_refusal`、且**一个路由字段名都不许多写在 `POST` 里** —— 否则这张表退化成文档，而文档正是 R1 写下的那种东西。
- **门覆盖按表循环**，不按记忆点名：16 行逐行喂"局域网 + 无令牌"（要求 403 **且那一行的方法 spy 一次都没响**），再逐行带令牌（要求每行真的被服务到）。另有一条"这个循环 ≥16 行"，防止哪天把表读成空元组、于是"全部端点都验过了"成为一句真空话。
- 其余：拒绝文案的累积顺序、两字段 precedence、未知门 fail closed、fall-through 三种形状（只有 helper 字段 / 拼错的 `clear-logg` / 空 POST）都不许回 success、上传侧三条。
- **页面侧对齐**：从 `setting.html` 抽出它能 POST 的每个字段名，要求在表里都登记得到 —— 这是"新增按钮悄悄落到一个不认识它的分发器上"唯一能被机器抓住的时刻。

**四个变异体各自红 3 / 9 / 3 / 1 条**：A 把未识别请求的返回值改回"success"（fall-through 那条谎装回去）→
3 条；B **删掉表里的一行**（`set-module-setting`）→ 9 条（它同时打掉派生名单、按表循环、页面侧对齐三处，
说明"少一条"在结构上确实可见）；C 把 `mirror-action` 的门从 `GATE_TOKEN` 降成 `GATE_MANAGEMENT` → 3 条；
D 让未知门名**放行**而不是拒绝（fail open）→ 1 条。每一个都只在属于自己的那句话上红，
还原后 `protocol.py` 与改动前逐字节一致（md5 比对）。
其中一个（B）还抓出的是**测试自己的**休眠分支：
`gate_of()` 当时读模块而非类，`FILE_ROUTES` 那条回退路径只有被扫到才 `AttributeError` ——
已改读 `protocol.Handler.FILE_ROUTES`，并补一条 `gate_of('plugin-file') == GATE_CODE` 让这条分支每次跑都被经过。

**没做的两件事（明确留下）**：① **`Handler` 类这一趟反而变大了**（`1250→2096` ≈846 行 →
`1365→2345` ≈**980 行**），因为 16 个 `_post_*` 薄包装现在都住在类里，而旧代码的分支体是塞在
一个函数里的 —— 行数没省，**省的是"读一处就知道所有门"这件事**。验收标准第 2 条（<300 行）
没有达成，方向甚至是反的；下一步该做的是把这个类**按面拆开**（管理 API / DLNA 收流 / 页面资源），
那是独立一步，别和改判定混在一次提交里；
② GET 侧的查询门控名单**故意没并进这张表**（它没有"漏一条就落代码"的同一种后果，且并进来会把
一次可评审的改动变成两次）。验收标准的第 1 条（无单文件 >3k 行）同样仍未达成 ——
`protocol.py` 2,447 / `screen_mirror.py` 8,065 / 套件 18,089 / `setting.html` 2,945。
...
见 `docs/risk-map.dot`（Graphviz 源，`rankdir=TB`）。核心可读结论：**R2 的"门控散落在 if/elif"是 R1 的结构性成因**（这一条 2026-09-25 已收进 `Handler.POST_ROUTES`，见上面 R2「已修」一节）；R3/R4 同属"跨线程共享状态缺一致性纪律"一族；R5 让 R1/R3/R4 这类"真网络/真线程/真硬件才暴露"的问题**系统性逃过套件**。

## 建议的下一步（顺序）

1. ~~**先坐实 R1**~~ —— **已完成（2026-09-25）**：drive-by PoC 在真实例上 4/4 命中，修复后 9/9（两种攻击形状被拒、两种合法形状仍放行），套件 Part 48 37 条 + 四个变异体，真浏览器四条流程验收。见上面「已修」一节。
2. ~~把 `Handler.POST` 改成声明式路由表（同时收 R1 的可审计性与 R2）~~ —— **已完成（2026-09-25）**：`Handler.POST_ROUTES` 16 行 + `FILE_ROUTES` + `GATE_REFUSALS`，`POST()` 只做 `_same_site()` 和按表走一遍；顺带修掉"未识别请求回 success"这条沉默的谎。套件 Part 51 24 条 + 四个变异体，见上面 R2「已修」一节。
