...
是协议类插件，可与任意渲染器共存。
**电脑投屏不属于这一栏**：它不需要先"切到 Screen Mirror"，菜单栏那一项在任何渲染器下
都点开同一个页面（详见 §1）。
...
菜单栏点一项 **「电脑投屏…」** 就在浏览器里打开设置页的「电脑投屏」页签
（`http://127.0.0.1:<端口>/?page=13`）。它不属于「选择播放器」那一栏：当前用哪个渲染器
都点得开，也不需要先把 Screen Mirror 选中。镜像进行中菜单里会追加 **「停止电脑投屏」**；
停止留在菜单里是故意的 —— 一个可能在另一台机器上被打开、可能被最小化的网页，
不能是唯一一条从「正在采集你的桌面」里出来的路。
...
- **先问协议，再问设备**：「投屏方式」列出这台机器的插件**能驱动的每一种协议**，
  ...
  「没有发现 DLNA 电视：局域网里只有这台 Mac 自己的接收端，已排除（搜于 …）」/
  「正在搜索…」/ 搜索本身失败的原因。其中「只有这台 Mac 自己」这一种值得单独说：Macast 自己就是一个
  DLNA 接收端，所以电视关机时 SSDP 往往**仍有应答**，而应答的是这台 Mac —— 以前
  它和"局域网彻底没人"都印成同一句「没有发现」，于是搜索到底坏没坏只能靠猜。
...
2. 在这一行（或下面的「设备」列表）里选一台。该行右侧会写清这一轮搜索的结论：
   ...
   点「重新搜索」重来一次，首次搜索要一两秒，页面不会假称搜索完成。
