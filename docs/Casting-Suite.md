...
| **Tk 8.6 以上的 Python** | 「电脑投屏」的控制台窗口是 Tk，而它跑在**独立进程**里（这样 Macast 自带的那份 Python 有没有 Tk 都不影响主程序） | macOS：`brew install python-tk`（配合 brew 的 python3）；Windows：重装 Python 时勾上 *tcl/tk and IDLE*；Linux：`apt install python3-tk`（Arch `pacman -S tk`）。**macOS 自带的 `/usr/bin/python3` 不算**——它的 Tk 停在 8.5.9，而那份 Tk 在新系统上只画得原生按钮：窗口会打开，里面一片空白，且没有任何报错。所以启动器宁可不打开窗口，也会在通知里指名要装什么（菜单栏其余投屏项目照样点得动，浏览器观看地址也照样能用） |
...
是协议类插件，可与任意渲染器共存。
**电脑投屏不属于这一栏**：v0.10 起它不需要先"切到 Screen Mirror"，菜单栏那一项在
任何渲染器下都点得开同一个控制台窗口（详见 §1）。
...
菜单栏点一项 **「电脑投屏…」** 就开窗 —— v0.10 起它不属于「选择播放器」那一栏，
当前用哪个渲染器都点得开，也不需要先把 Screen Mirror 选中。镜像进行中菜单里会追加
状态行与「停止电脑投屏」；停止留在菜单里是故意的 —— 一个会被最小化、会被别的窗口
盖住的窗口，不能是唯一一条从「正在采集你的桌面」里出来的路。

其余全在控制台窗口里（`macast/mirror_console.py`，1080×700，深色，三平台同一套色板）：

```
┌ 电脑投屏  Screen Mirror v0.10            [ 未镜像 ] [ 开始镜像 ] ┐
│ 左栏（区块按当前协议增减）        │ 右栏                          │
│   投屏方式：每种协议一行 ——       │   观看地址（浏览器协议才有）     │
│    Chromecast · 低延迟 · DLNA 电视│    ＋复制 / 在本机浏览器打开       │
│    · 浏览器，各自写着自己那轮搜索  │   这台电脑的画面：约 1 fps 快照    │
│    的结论，行内就能选设备         │                               │
│   设备（选中协议的机器列表）       │   动态：连接、断开、档位回退、     │
│    ＋重新搜索                    │    权限被拒、编码器退出（最近20条）│
│   需要安装（缺什么才出现）         │                               │
│   兼容档位（仅 DLNA 协议）        │                               │
│   画质（四档）                   │                               │
│   这台电脑：显示器 / 指针 / 硬件编码│                               │
...
- **先问协议，再问设备**：「投屏方式」列出这台机器的插件**能驱动的每一种协议**，
  各带自己那轮搜索的结论 —— 「发现 2 台」/「没有发现 Chromecast（搜于 3 秒前）」/
  「正在搜索…」/ 搜索本身失败的原因。空的 Chromecast 答案不代表 DLNA 那边也没有电视，
  浏览器协议根本不问设备（那一行写「不需要设备」）。以后自研一种接收协议，
  是**加一行、加一个搜索**，窗口不用改。
- **需要装的东西不会被通知吃掉**：任何插件说「去装 X」，同一句话（含可以复制的命令）
  会留在「需要安装」卡片上，直到那个依赖真的被满足 —— 通知五秒就没了，这张卡不会。
- **区块是按需长出来的**，不是"全列出来再灰掉"：Windows 没有 BlackHole 可装，浏览器协议
  没有兼容档位可选 —— 永远点不动的行看起来像坏掉的窗口。
- **改任何一项都是"提交后重读"**：档位可能被看门狗自己换掉、编码器可能刚死，所以没有乐观更新，
  窗口每秒读一次状态。
- **预览是快照不是视频**：约 1 fps 向应用要一张 480px 宽的桌面图（`mirror-snapshot`）。
  取不到时留着上一张并写明原因，绝不为了预览多开一路采集。
- **窗口只会有一个**：再点菜单项是把已经开着的窗口叫到前面，不是再开一个。
- 窗口的输出在 `~/Library/Application Support/Macast/mirror_console.log`
  （Tk 的 traceback 只有这一份记录），插件日志仍在 `logs/ScreenMirror.log`
  （设置页 →「日志」下拉里选）。
- 打不开窗口时通知会指名原因（"本机没有任何 Tk ≥ 8.6 的 Python" + 该装什么，或子进程退出码
  + 它留下的最后一行错误），不会是"点了没反应"；这句话同样会留在「需要安装」卡片上。
  macOS 上最常见的一条：**系统自带的 `/usr/bin/python3` 不算**，它的 Tk 停在 8.5.9，
  在新系统上只画得动原生按钮，开出来的窗口是空白的，所以启动器宁可拒绝它。
...
2. 在这一行（或下面的「设备」列表）里选一台。该行右侧会写清这一轮搜索的结论：
   「发现 N 台」、「没有发现 Chromecast（搜于 …）」、或搜索为什么没跑起来；
   点「重新搜索」重来一次，首次搜索要一两秒，窗口不会假称搜索完成。
...
| 平台 | 状态 | 首次设置 |
|---|---|---|
| **macOS** | 需要 BlackHole 虚拟声卡（**FFmpeg 发行版至今抓不到 mac 系统音频**） | 窗口「系统声音」→「一键设置」。它**先判断你的机器处在哪一格**（下表），再决定做什么：只有"真的没装"才下载官方 `.pkg` → **弹出图形安装器，你输一次密码**（`.pkg` 无法静默安装，别期待全自动）→ 等设备出现 → 建/复用名为 `Macast Screen Mirror` 的多输出聚合设备 → 把默认输出切过去。**面板里那 11 步行是实时推进的**（跳过的行不进分母，进度条永不回退），也可以点「打开详细进度页」在浏览器里看同一份进度。**任何一步失败就降级**：打开「音频 MIDI 设置」+ 文字指引。装完这一行会出现「恢复原声音输出」按钮，走它回去 |
| **Linux** | PulseAudio 直接有 tap | 采集口是 `<sink>.monitor`，插件自动探测；Wayland 下**没有画面可采**（x11grab  only），会明说 |
| **Windows** | 仅画面 | 这一版不做 Windows 音频，窗口「系统声音」那一行会写明「Windows 下仅画面」 |
...
| 局域网里有没有东西可投 | 真发一次 mDNS browse（`_googlecast._tcp` / `_airplay._tcp`）和一次 SSDP `MediaRenderer` 探测。**没搜到电视的时候，"控制台窗口里设备列表是空的"就不是插件的问题** |
