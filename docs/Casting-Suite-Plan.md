...
**P6 剩下的**：版本号两处（`macast/.version` + `macast/_version.py`）+ tag + 产物验证 —— **发版要先问用户**（本机应用在跑 0.7.14）。端到端回归已从"剩下的"里划掉。
