...
### 6.2 P6 第五批明细（commit `0996789`，已推送）：「永远装不完」的根因不是"没重启"
...
**仍未验证 / 要说清的**：① 用户这台机器的 `BlackHole2ch.driver` **还需要那一次重载**
（或重启一次电脑）才会真的可用 —— 动他的 coreaudiod 要他的管理员密码，属于要先问的操作；
