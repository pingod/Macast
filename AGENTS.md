...
| `verify_cast_airplay.py` | **主验证套件**（本机 1046 条 —— 它随数据规模伸缩：索引清空后，按条目循环的那些用例
...
| `cast_streaming_probe.py` | **把 `macast/plugins/renderer/screen_mirror.py` 的低延迟通道原样打到真电视上**（镜像接收器 `0F5096E8` + OFFER/ANSWER + UDP RTP）。它 `import` 插件本体而不是复刻协议，所以真机通过 = 菜单栏那条路通过。`--live` 采真桌面、`--source` 用文件、`--dump` 留 Annex-B 给 ffprobe。Part 24 只证明字节自洽，**这一步才证明电视认不认**（§4.9） |
