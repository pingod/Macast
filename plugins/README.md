...
| `screen_mirror.py` | **Screen Mirror v0.12** — 把桌面屏幕实时镜像到局域网：**两条 Chromecast 通道**（兼容 LOAD / 实验性低延迟 Cast Streaming，后者设备不认就自动回落，回落那一路是有声音的）、**没有 Google 栈的老电视（DLNA）**、或任意浏览器打开一个网址（三平台，macOS 一键装好系统声音：带进度页的步骤机，v0.9 起按机器状态判定，**不会重复下载已装好的驱动**；v0.12 起还会说清"有采集口"与"声音真的路由进去了"是两件事）。v0.11 起所有控制项都在设置页的「电脑投屏」页签里（v0.10 那个桌面 Tk 窗口已删除），**v0.12 起菜单栏一项投屏控制都不放**（2026-09-23 所有者定）：停止走页签、或「停止接受投屏」、或切走渲染器，三条路是同一次收尾 | 依赖用户自己装的 `ffmpeg` 命令；macOS 上还要屏幕录制权限（缺了会指名）。页签就是 Macast 自带的那个网页，**不需要额外的 python、Tk 或任何 GUI 库** |
...
- **Screen Mirror**：「电脑投屏」页签的「投屏方式」先选一类，再点「开始镜像」。链路是 ffmpeg 屏幕采集 →
  ...
  **v0.5 的 DLNA 电视目标**（给没有 Google 栈、也不开浏览器的那类电视：老智能电视、
  ...
  推流前先攒够**约 6 秒画面**（按档位码率折算，夹在 3–8 MiB；页签上的状态行与开始提示
  都会说出这个秒数），所以这条链路是
  ...
  **v0.6 的第二条 Chromecast 通道 —— 「Chromecast 低延迟（实验 · 此通道无声音）」**：
  ...
  ① 这条通道**本身没有声音**（镜像接收器的音频流是下一步）；② 单文件插件不能装加密库，
  ...
  开始的那条通知会写明「设备拒绝了低延迟通道（…），已改用兼容通道 LOAD，这一路有声音」。
  **这一条曾经被反过来读成 bug**：所有者报「这个我试过了，是有声音的」的时候，界面上那一行
  只写着「实验 · 无声音」——真相是电视拒绝了这一通道、会话其实跑在 LOAD 上。
  现在标签、提示与通知三处说的是同一件事，分辨方法是看状态行：低延迟通道报「在途 / 丢帧」，
  LOAD 通道报「N 个观看端 / 丢块」。想自己拿真机验一遍：
  ...
  **v0.7 把上面那条「一键设置」变成了一个看得见的步骤机**（`AUDIO_STEPS` 十步：环境自检 →
  ...
  → 建/复用多输出设备 → 切默认输出）。之前它只在终端里打日志，你点了「一键设置」之后面对的是一个
  ...
  **v0.10 曾把整张菜单搬进一个桌面 Tk 窗口，v0.11 又把它搬进了设置页**（布局规则在
  ...
  投出去了吗、电视看到的是什么 —— 一点位置都没有。**v0.12 起菜单栏连「电脑投屏…」和
  「停止电脑投屏」两行也没有了**（2026-09-23 所有者定：菜单只留通用功能）。v0.11 那句
  「停止必须留在菜单里」的理由没有被推翻，只是换了承接者：控制面是一个可能在另一台机器上打开、
  可能被最小化的网页，所以从「正在采集你的桌面」里出来仍有三条路 —— 页签上的「停止镜像」、
  菜单栏「停止接受投屏」（停掉整个服务）、以及「设置 → 选择播放器」切走渲染器，
  后两条触发的就是 `ScreenMirrorRenderer.stop()`，与那颗按钮是同一次收尾。
  三条实现约束，改代码前先读：
