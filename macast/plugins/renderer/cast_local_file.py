...
# <macast.desc>把本机磁盘上的文件投到 Chromecast / Google TV 或 DLNA 电视：在菜单栏选文件夹、点文件，就在电视上播放。设备能原生解码的文件由内置的 Range/206 HTTP 服务按字节直供，遥控器的暂停和拖动作用的是真实文件。其余格式（AVI、MKV、HEVC、DTS、AC-3、第二条音轨、内嵌字幕）由 ffmpeg 边播边转成 MPEG-TS，拖动则在新位置重启编码器。另有：电视上报上一集播完就自动连播的播放列表；转码路径上可选音轨并调音画同步偏移；内嵌字幕对 Chromecast 转成 WebVTT 重新提供（本机 ffmpeg 带 libass 时烧进画面）；被别的发送端抢占时看门狗把电视抢回来；真正发 QUIT_APP 让电视回到信号源而不是停在最后一帧；不碰任何文件直接投一个普通网址；以及在本机有采集口时把系统声音单独以音频档投出去。</macast.desc>
#
