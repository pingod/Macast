...
class NVAConectionBaseHandler:
    ...
    def send_res(self, counter, data=None):
        ...
        if data is None:
            binary = struct.pack(">cBI", RET_CMD, 0, counter)
        else:
    ...
    def send_ping(self):
        ...
        with self.counter_lock:
            self.counter += 1
        binary = struct.pack(">cBI", PING, 0, self.counter)
        self.send(binary)
            ...
            try:
                ...
                if data[index] == SEND_CMD[0]:
                    ...
                    if all_l == 0:
                        ...
                        if length - index - 6 < 4:
                            break
                        param_l = struct.unpack('>I', data[index + 6:index + 10])[0]
...
class SelectPoller(object):
    ...
    def poll(self):
        ...
        try:
            ...
            # A signal interrupting select() (EINTR) is not an error; report
            # "nothing ready" and let the caller poll again.
            return []
        return r
...
class NVAProtocol(DLNAProtocol):
    ...
    #: This protocol's handler must win the CherryPy tree root whenever it is
    #: enabled, even alongside DLNA. `NVAHandler` subclasses `DLNAHandler`, so
    #: it serves every UPnP route DLNA needs *plus* the SETUP / RESTORE /
    #: STARTRESTORE endpoints and the NirvanaControl SCPD that the Bilibili TV
    #: client speaks. Without this the group mounted whichever SSDP protocol
    #: came first in the list -- DLNA -- and every NVA route was unreachable,
    #: silently. See ProtocolGroup.primary.
    handler_priority = 10
